...
from herring.herring_app import HerringApp, ArgumentHelper
...
class TestHerring(TestCase):
    ...
    def setUp(self):
        ...
        self.herring = HerringApp(self._output_buf)
    ...
    def test__tasksToDependDict(self):
        ...
        dataDict = self.herring._tasks_to_depend_dict(self.herring_tasks.keys(),
                                                      self.herring_tasks)
    ...
    def verify_findDependencies(self, src_list, dest_list):
        ...
        """
        depends = self.herring._find_dependencies(src_list, self.herring_tasks)
    ...
    def verify_resolveDependencies(self, src_list, dest_list):
        ...
        :param dest_list: output list containing task names
        """
        depends = self.herring._resolve_dependencies(src_list,
                                                     self.herring_tasks)
        self.assertEquals(depends, dest_list)
    ...
    def test__wrap(self):
        ...
        for width in range(40, 80):
            lines = self.herring._wrap(quotes, width).split("\n")
            for line in lines:
